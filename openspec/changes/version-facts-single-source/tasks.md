...
- [x] 1.1 Confirm the working base is clean and on the change branch; no edits yet.
- [x] 1.2 Prove build determinism: run `npm run build` twice, hashing `dist/index.html` and `dist/es/index.html` each run with `shasum -a 256`. If the two runs disagree, STOP and report — the diff would be meaningless.
- [x] 1.3 Save the two baseline hashes AND copies of both built files to a scratch path outside the repo.
- [x] 1.4 Record the baseline `npx playwright test` count (expect 28 passed).

## Phase 2: Foundation

- [x] 2.1 Create `src/data/versions.ts` with `ReleaseFact`, module-local `goModulePath`, exported `releases` (`as const satisfies Record<'stable'|'prerelease', ReleaseFact>`), and exported `prereleaseInstall`. Do not export `goModulePath`.
- [x] 2.2 Run `npm run check` — 0 errors / 0 warnings / 0 hints.

## Phase 3: Bind EN (parallel-safe with Phase 4)

- [x] 3.1 `DocumentationContentEn.astro`: add a three-line frontmatter fence importing `{ prereleaseInstall, releases }` from `../data/versions`, directly above `<section class="hero">` with NO blank line between `---` and `<section`. This is the only new parse-mode surface and the expected failure shape for Phase 5.
- [x] 3.2 Line 5: bind both chips — `<b>{releases.stable.version}</b>`, `<b>{releases.prerelease.version}</b>`.
- [x] 3.3 Line 1400: bind `<code>{releases.stable.version}</code>` and `<span class="pill">{releases.stable.released}</span>`. Leave the `@latest` command literal.
- [x] 3.4 Line 1401: bind version, `released`, and `<code>{prereleaseInstall}</code>`. Leave line 1402 (`@main`) untouched.

## Phase 4: Bind ES (parallel-safe with Phase 3)

- [x] 4.1 `DocumentationContentEs.astro`: put the same import BETWEEN the existing `---` lines 1-2. Do not touch `{<>` (line 4) or `</>}` (line 1501).
- [x] 4.2 Line 9: bind both chips; keep `Estable` / `Última RC` authored.
- [x] 4.3 Lines 1425-1426: bind exactly as EN; keep `Prerelease` / `Desarrollo` authored and line 1427 untouched.
- [x] 4.4 Re-read all six edited lines: every `{expr}` must be immediately preceded by `>` and followed by `<`, no whitespace. Never split a partial text node.

## Phase 5: Verification

- [x] 5.1 `npm run check` — 0 / 0 / 0.
